...
class SOX:
    ...
    def train(self, num_iters, batch_size=None, lr=1e-4, weight_decay=10):
        ...
        for t in range(num_iters):
            ...
            denom = (1 - self.gamma) * self.u_all[index] + self.gamma * all_loss
            numer = (1 - self.gamma) * self.u_pos[index] + self.gamma * pos_loss
            ...
            self.u_all[index] = (1 - self.gamma) * self.u_all[index] + self.gamma * all_loss.detach().clone()

            self.u_pos[index] = (1 - self.gamma) * self.u_pos[index] + self.gamma * pos_loss.detach().clone()
