...
def main(args):
    ...
    for seed in range(args.num_seeds):
        ...
        if args.dataset == 'mnist':
            ...
            raise ValueError('Unknown dataset')
        moap = MOAP(X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test, device=device,
                  emb_dim=emb_dim, init=args.init, gamma=args.gamma, beta=args.beta, seed=seed)
        test_loss_trace = moap.train(num_iters=args.num_iters, batch_size=args.batch_size, lr=args.lr, weight_decay=args.wd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--emb_dim", type=int, default=32, help="Dimension of embedding.")
