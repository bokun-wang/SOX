...
def main(args):
    ...
    for seed in range(args.num_seeds):
        ...
        moap = MOAP(X_train_ps=X_train_ps, X_train_ns=X_train_ns, X_test_ps=X_test_ps, X_test_ns=X_test_ns, device=device,
                gamma=args.gamma, beta=args.beta, seed=seed)
        test_loss_trace, test_auc_trace, total_time = moap.train(num_iters=args.num_iters, batch_size_out=args.batch_size_out,
                                        batch_size_in=args.batch_size_in, lr=args.lr, weight_decay=args.wd)
    ...
    print("MOAP: avg time: {:.5f}, std: {:.5f}, avg te loss {:.5f}, std: {:.5f}".format(np.average(all_total_time),
                                                                                        np.std(all_total_time),
                                                                                        np.average(all_final_loss),
                                                                                        np.std(all_final_loss)))
    file_name = "./results/MOAP-{}_traces.npz".format(args.dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="ijcnn1", help="dataset.")
    ...
    parser.add_argument("--gamma", type=float, default=0.9, help="Avg.")
